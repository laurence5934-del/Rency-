...
def show_portfolio_intelligence(
    ...
    *,
    ...
    if not summary_rows:
        st.warning(
            "Portfolio intelligence is unavailable because no "
            "IBKR account summary was received."
        )
        return
    ...
    row1[0].metric(
        "Net Liquidation",
        _money(snapshot.net_liquidation),
    )
    row1[1].metric(
        "Available Funds",
        _money(intelligence.available_funds),
    )
    row1[2].metric(
        "Buying Power",
        _money(snapshot.buying_power),
    )
    row1[3].metric(
        "Cash",
        _money(intelligence.total_cash),
    )

    row2 = st.columns(4)
    row2[0].metric(
        "Today's P/L",
        _money(snapshot.daily_total_pnl),
    )
    row2[1].metric(
        "Exposure",
        _percent(intelligence.exposure_percent),
    )
    row2[2].metric(
        "Open Positions",
    ...
    row3[0].metric(
        "Capital Utilization",
        _percent(intelligence.utilization_percent),
    )
    row3[1].metric(
        "Cash Percentage",
        _percent(intelligence.cash_percent),
    )
    row3[2].metric(
        "Margin Multiple",
        _multiple(intelligence.margin_multiple),
        help=(
            "Buying power divided by net liquidation. "
            "This is clearer than a percentage above 100%."
        ),
    )
    row3[3].metric(
        "Drawdown",
        _percent(snapshot.drawdown_percent),
    )

    st.caption(
        "Available margin estimate: "
        f"{_money(intelligence.available_margin)}"
    )
    ...
    with advisor_left:
        ...
        st.metric(
            ...
            _money(
                intelligence.advisor.suggested_position_size
            ),
        )

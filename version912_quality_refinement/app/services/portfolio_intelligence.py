...
Builds a dashboard-ready portfolio view from the existing IBKR account
summary, positions, snapshot adapter, and portfolio risk limits.
This module never transmits orders.
"""
...
class PortfolioIntelligence:
    ...
    @property
    def buying_power_percent(self) -> float:
        """Backward-compatible value retained for API consumers."""
        if self.snapshot.net_liquidation <= 0:
            return 0.0
        return (
            self.snapshot.buying_power
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "largest_position_value": round(
                self.largest_position_value,
                2,
            ),
            "largest_position_percent": round(
                self.largest_position_percent,
                2,
            ),
            ...
            "advisor": {
                ...
                "suggested_position_size": round(
                    self.advisor.suggested_position_size,
                    2,
                ),
                "can_open_new_position": (
                    self.advisor.can_open_new_position
                ),
...
def _calculate_health_score(
    ...
    if limits.maximum_portfolio_exposure_percent > 0:
        exposure_ratio = (
            exposure_percent
            / limits.maximum_portfolio_exposure_percent
        )
        if exposure_ratio > 1:
            score -= 35
        elif exposure_ratio >= 0.9:
            score -= 22
    ...
    if limits.maximum_drawdown_percent > 0:
        drawdown_ratio = (
            ...
            / limits.maximum_drawdown_percent
        )
        if drawdown_ratio >= 1:
            score -= 35
    ...
    if snapshot.open_position_count >= limits.maximum_open_positions:
        score -= 18
    elif snapshot.open_position_count >= max(
        limits.maximum_open_positions - 2,
        1,
    ):
        score -= 8
...
def _build_advisor(
    ...
    positions_ok = (
        ...
        < limits.maximum_open_positions
    )
    buying_power_ok = (
        snapshot.buying_power
        > limits.minimum_buying_power
    )
    daily_loss_ok = (
        snapshot.daily_total_pnl
        > -abs(limits.maximum_daily_loss)
    )
    drawdown_ok = (
        snapshot.drawdown_percent
        < limits.maximum_drawdown_percent
    ...
    checks.append(
        ...
        if exposure_ok
        else "Exposure limit is reached."
    )
    checks.append(
        "Open-position capacity is available."
        if positions_ok
        else "Open-position limit is reached."
    )
    checks.append(
        "Buying power reserve is healthy."
        if buying_power_ok
        else "Buying power reserve is too low."
    )
    checks.append(
        "Daily loss limit is clear."
        if daily_loss_ok
        else "Daily loss limit is reached."
    )
    checks.append(
        "Drawdown is within limits."
        if drawdown_ok
        else "Maximum drawdown is reached."
    )
    ...
    if can_open and health_score >= 70:
        recommendation = (
            "Portfolio limits permit one additional paper-trade "
            "position, subject to symbol-level risk approval."
        )
    elif can_open:
        recommendation = (
        ...
        recommendation = (
            "Do not open another position until the failed "
            "portfolio checks are resolved."
        )
        suggested_size = 0.0
...
def build_portfolio_intelligence(
    *,
    ...
    exposure_percent = _percent(
        snapshot.total_position_value,
        snapshot.net_liquidation,
    )
    cash_percent = _percent(
        total_cash,
        snapshot.net_liquidation,
    )
    ...
    available_margin = max(
        snapshot.buying_power - snapshot.total_position_value,
        0.0,
    )
